...
not share keys**: a dev-issued key is rejected by prod (measured: `401 "Invalid internal API
key"`). If a user's key stops working right after you point them back at production, this is
why — they need a production key, not just the production URL.
