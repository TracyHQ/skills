...
description: Use to create an AI Visibility Report for a Shopify store, run entirely on the user's own machine and their own API keys. Collection, detection and analysis happen locally; the Mention Network MCP is used only to fetch the live catalog and to store the finished report. Each of the four engines (chatgpt, claude, gemini, google_ai_mode) is an API key, and a missing one is offered as a choice — supply it now or skip that engine — never scored as a silent zero. Accepts a one-line shorthand (`/create-visibility-report kbeautyarabia.com byok country=SA`).
