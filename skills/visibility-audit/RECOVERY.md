...
| Symptom | Cause | Fix |
|---|---|---|
| `MCP HTTP 401: ...` on `validate_byok_website_audit` / `submit_byok_website_audit` | no `MENTION_NETWORK_KEY`, or a wrong one, and the server did not serve this call as `anonymous` | `submit-audit.mjs` no longer refuses to run without a key — it calls out through `mcp-client.mjs`, which omits `Authorization` entirely rather than erroring locally (verified prod behaviour since 2026-08-19: unauthenticated `tools/list`/`describe_check_grid` return 200; a write like this one may or may not be). A `401` here is the server's answer, found out by running the step, not something to guess in advance. First `set -a; . "$CREDS"; set +a` if a key IS stored but this shell never sourced it. If there is genuinely no key, **offer one** (`credentials.mjs save MENTION_NETWORK_KEY`) — but `audit.json`/`report.md` are already complete and already the deliverable; only skip the retry if the user chooses `no-save` or accepts the local-only result |
| A stored key still gets `401` | the key is wrong or revoked — a stale key behaves *worse* than none, since the server rejects it outright rather than trying `anonymous` | `credentials.mjs check MENTION_NETWORK_KEY` will say "not probed here; P1 verifies it against the MCP" — this submit attempt IS that verification. Replace or `remove` the key rather than retrying the same one |
