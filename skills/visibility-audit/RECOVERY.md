...
| `[AGGREGATE_MISMATCH]` | the server's re-computed score differs from ours by more than 1 | **this skill's framework has drifted from the backend's.** Do not force it through: run `node --test scripts/tests/framework-drift.test.mjs` against a checked-out backend, re-port `framework.mjs`, re-score. Note this code only ever catches *weight* drift; a scorer that grades the same page differently submits cleanly and silently, which is what `scripts/tests/scorer-parity.test.mjs` is for |
