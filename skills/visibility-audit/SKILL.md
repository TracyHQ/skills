...
description: Use to run an AI-visibility audit — the GEO audit of a Shopify store's product page — on the user's own API keys, so the Mention Network backend spends nothing. Grades 40 criteria with the same framework and weights the backend uses, adds off-store signals with the user's SerpApi key, writes the diagnosis with their own LLM key, and returns a hosted PDF alongside a local audit.json and report.md. A missing lane is a setup step, never a silent zero. Accepts a one-line shorthand (`/visibility-audit kbeautyarabia.com product=water-bank llm=anthropic`). Use `create_website_audit` over the MCP instead when the backend should pay.
